...
// DOMHighResTimeStamp that can be compare with performance.now().
// This becomes a no-op for browsers that provide high resolution
// event timestamp.
//
// Usage example:
...
getHighResTimeStamp = (function(){
  ...
  // The offset value to system startup clock that is being used
  // by Firefox as timebase for all input events.
  ...
  function getTimebaseOffset(event) {
    if (event.timeStamp >= dateNowAtLoad) {
      ...
      if (event.timeStamp <= now) {
        ...
        // Compute system startup clock offset which is the difference between current
        // time in high resolution clock vs system startup clock.
        // We use the passed in event timestamp as an approximation of the current
        // time in the  system startup clock.
